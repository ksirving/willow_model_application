...
save(new_data, file="output_data/W1_F319_seedling_inundation_discharge_probability_time_series_red_columns.RData")
...
save(new_data, file="output_data/W1_F319_seedling_inundation_discharge_probs_2010_2017_TS.RData")
...
write.csv(limits, "output_data/W1_F319_seedling_depth_Q_limits.csv")

# plot discharge points ---------------------------------------------------
labels <- c(depth_cm_LOB = "Left Over Bank", depth_cm_MC = "Main Channel", depth_cm_ROB = "Right Over Bank")

png("Figures/Application_curves/Depth/F319_willow_seedling_depth_prob_Q_thresholds.png", width = 500, height = 600)
...
ggplot(new_data, aes(x = Q, y=prob_fit)) +
  ...
  labs(title = "F319: seedling/Depth: Probability ~ Q",
       y = "Probability",
...
write.csv(melt_time, "output_data/W1_willow_F319_seedling_depth_time_stats.csv")

## plot for annual stats - need probs in order

png("Figures/Application_curves/Depth/F319_seedling_depth_perc_time_above_threshold_annual.png", width = 500, height = 600)
...
png("Figures/Application_curves/Depth/F319_seedling_depth_perc_time_above_threshold_non_critical.png", width = 500, height = 600)
...
png("Figures/Application_curves/Depth/F319_seedling_depth_perc_time_above_threshold_critical.png", width = 500, height = 600)
...
write.csv(total_days, "output_data/W1_F319_willow_seedling_depth_total_days.csv")
...
## save df
write.csv(melt_days, "output_data/W1_F319_willow_seedling_total_days_long.csv")

library(scales)

## plot all ts

png("Figures/Application_curves/Depth/F319_seedling_depth_lob_rob_mc_no_days_within_Q.png", width = 500, height = 600)
...
png("Figures/Application_curves/Depth/F319_seedling_depth_lob_rob_mc_no_days_within_Q_by_year.png", width = 500, height = 600)
...
ggplot(melt_days, aes(x =month_year, y=n_days)) +
  ...
  scale_y_continuous(limits= c(0, 31)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(title = "F319: Number of days within discharge limit in relation to Depth: Mid Channel",
       y = "Number of days per Month",
...
png("Figures/Application_curves/Depth/F319_seedling_depth_lob_rob_mc_no_days_within_Q_by_season.png", width = 500, height = 600)
...
png("figures/Application_curves/Shear/F319_seedling_shear_perc_time_above_threshold_annual.png", width = 500, height = 600)
...
ggplot(melt_time_ann, aes(x = water_year, y=value)) +
  ...
  facet_wrap(~position, scales="free_x", nrow=3) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(title = "F319: Time within discharge limit in relation to Shear Stress (Annual)",
       y = "Time (%)",
       x = "Year") #+ theme_bw(base_size = 15)
dev.off()
## plot for winter stats - need probs in order
...
png("figures/Application_curves/Shear/F319_seedling_shear_perc_time_above_threshold_non_critical.png", width = 500, height = 600)
...
ggplot(melt_time_winter, aes(x = water_year, y=value)) +
  ...
  facet_wrap(~position, scales="free_x", nrow=3) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(title = "F319: Time within discharge limit in relation to Shear (Non_critical)",
       y = "Time (%)",
       x = "Year") #+ theme_bw(base_size = 15)
dev.off()
## plot for summer stats - need probs in order
...
png("figures/Application_curves/Depth/F319_seedling_shear_perc_time_above_threshold_critical.png", width = 500, height = 600)
...
ggplot(melt_time_summer, aes(x = water_year, y=value)) +
  ...
  facet_wrap(~position, scales="free_x", nrow=3) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(title = "F319: Time within discharge limit in relation to Shear (critical)",
       y = "Time (%)",
       x = "Year") #+ theme_bw(base_size = 15)
