...
write.csv(limits, "output_data/W1_LA11_seedling_depth_Q_limits.csv")
...
write.csv(limits, "output_data/W2_LA11_seedling_shear_Q_limits.csv")

#### plot
png("figures/Application_curves/Shear/LA11_seedling_shear_prob_Q_thresholds.png", width = 500, height = 600)
...
ggplot(new_data, aes(x = Q, y=prob_fit)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none") +
  labs(title = "LA11: Willow Seedling/Shear: Probability ~ Q",
       y = "Probability",
       x = "Q (cfs)") #+ theme_bw(base_size = 15)
...
write.csv(melt_time, "output_data/W2_LA11_willow_seedling_shear_time_stats.csv")
...
png("figures/Application_curves/Shear/LA11_seedling_shear_perc_time_above_threshold_annual.png", width = 500, height = 600)
...
ggplot(melt_time_ann, aes(x = water_year, y=value)) +
  ...
  facet_wrap(~position, scales="free_x", nrow=3) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(title = "LA11: Time within discharge limit in relation to Shear Stress (Annual)",
       y = "Time (%)",
       x = "Year") #+ theme_bw(base_size = 15)
dev.off()
## plot for winter stats - need probs in order
...
png("figures/Application_curves/Shear/LA11_seedling_shear_perc_time_above_threshold_non_critical.png", width = 500, height = 600)
...
ggplot(melt_time_winter, aes(x = water_year, y=value)) +
  ...
  facet_wrap(~position, scales="free_x", nrow=3) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(title = "LA11: Time within discharge limit in relation to Shear (Non_critical)",
       y = "Time (%)",
       x = "Year") #+ theme_bw(base_size = 15)
dev.off()
## plot for summer stats - need probs in order
...
png("figures/Application_curves/Depth/LA11_seedling_shear_perc_time_above_threshold_critical.png", width = 500, height = 600)
...
ggplot(melt_time_summer, aes(x = water_year, y=value)) +
  ...
  facet_wrap(~position, scales="free_x", nrow=3) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(title = "LA11: Time within discharge limit in relation to Shear (critical)",
       y = "Time (%)",
       x = "Year") #+ theme_bw(base_size = 15)
...
write.csv(total_days, "output_data/W2_LA11_willow_seedling_shear_total_days.csv")
# # create year_month column       
total_days <- ungroup(total_days) %>%
  unite(month_year, water_year:month, sep="-", remove=F)
...
## save df
write.csv(melt_days, "output_data/W2_LA11_willow_seedling_shear_total_days_long.csv")
